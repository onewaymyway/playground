...
(function(window,document,Laya){
	...
	*/
	...
	var TestSocket=(function(){
		function TestSocket(){
			...
			Laya.init(1000,900);
			this.testDebugServer();
			Laya.loader.load("res/atlas/comp.atlas",new Handler(this,this.onResLoaded));
		}

		__class(TestSocket,'TestSocket');
		var __proto=TestSocket.prototype;
		__proto.onResLoaded=function(){
			this.initTestPage();
		}

		//initControlPage();
		__proto.initTestPage=function(){
			var tpg;
			tpg=new TestView();
			Laya.stage.addChild(tpg);
			console.log(RemoteWorkUtils.getAllTargetView());
			var tpg2;
			tpg2=new TestOrzUI();
			tpg2.pos(600,0);
		...
		__proto.initControlPage=function(){
			var controlPage;
		...
		__proto.test=function(){
			this.socket=new StockSocket();
			this.socket.channel="debug";
			this.socket.on("OnJoinedChannel",this,this.onJoinedChannel);
			this.socket.connect("ws://127.0.0.1:9909");
		}

		__proto.onJoinedChannel=function(){
			console.log("onJoinedChannel");
			debugger;
			this.socket.sendToChannel({"hihi":"orz"});
		}

		__proto.testDebugClient=function(){
			this.debugClient=new DebugClient();
			this.debugClient.connect("ws://127.0.0.1:9909");
		}

		__proto.testDebugServer=function(){
			this.debugServer=new DebugServer();
			this.debugServer.connect("ws://127.0.0.1:9909");
		}
	...
	*@author ww
	*/
	//class remotedebugtool.client.DebugMsgConst
	var DebugMsgConst=(function(){
		function DebugMsgConst(){}
		__class(DebugMsgConst,'remotedebugtool.client.DebugMsgConst');
		DebugMsgConst.GetAllDebugView="GetAllDebugView";
		DebugMsgConst.UpdateNodeProps="UpdateNodeProps";
		return DebugMsgConst;
	})()
	...
	*@author ww
	*/
	//class remotedebugtool.client.RemoteWorkUtils
	var RemoteWorkUtils=(function(){
		function RemoteWorkUtils(){}
		__class(RemoteWorkUtils,'remotedebugtool.client.RemoteWorkUtils');
		RemoteWorkUtils.walkTargetEX=function(target,fun,_this,filterFun){
			...
			for(i=0;i<len;i++){
				tChild=childs[i];
				RemoteWorkUtils.walkTargetEX(tChild,fun,_this,filterFun);
			}
		}
	...
	*@author ww
	*/
	//class remotedebugtool.common.MsgConst
	var MsgConst=(function(){
		function MsgConst(){}
		__class(MsgConst,'remotedebugtool.common.MsgConst');
		MsgConst.createMsg=function(type,data){
			var rst;
			rst={};
			rst.type=type;
			rst.data=data;
		...
		MsgConst.Hello="hello";
		MsgConst.JoinChannel="joinchannel";
		MsgConst.ToChannelOther="tochannelother";
		return MsgConst;
	})()
	...
	*@author ww
	*/
	//class remotedebugtool.client.DebugClient extends laya.events.EventDispatcher
	var DebugClient=(function(_super){
		function DebugClient(){
			this.socket=null;
			this.channel="debug";
			DebugClient.__super.call(this);
			this.socket=new StockSocket();
			this.socket.on("OnJoinedChannel",this,this.onJoinedChannel);
			this.socket.on("OnChannelMsg",this,this.onChannelMsg);
		}

		__class(DebugClient,'remotedebugtool.client.DebugClient',_super);
		var __proto=DebugClient.prototype;
		__proto.connect=function(server){
			(server===void 0)&& (server="ws://127.0.0.1:9909");
		...
		__proto.sendToChannel=function(obj){
			this.socket.sendToChannel(obj);
		}

		__proto.sendTypedToChannel=function(type,obj){
			this.socket.sendTypedToChannel(type,obj);
		}

		__proto.onChannelMsg=function(dataO){
			console.log("onChannelMsg:",dataO);
			var curData;
			curData=dataO.data;
			if (!curData)return;
			this.event("OnRemoteInfo",curData);
			this.dealChannelMsg(curData);
		}

		__proto.dealChannelMsg=function(dataO){
			switch(dataO.type){
				}
		}

		DebugClient.OnReady="OnReady";
		DebugClient.OnRemoteInfo="OnRemoteInfo";
		return DebugClient;
	})(EventDispatcher)
	...
	*@author ww
	*/
	//class remotedebugtool.client.StockSocket extends laya.events.EventDispatcher
	var StockSocket=(function(_super){
		...
		__class(StockSocket,'remotedebugtool.client.StockSocket',_super);
		...
		__proto.onMessage=function(msg){
			...
			switch (dataO.type){
				case "welcome":
					this.sendJson({type:"joinchannel",channel:this.channel});
					break ;
				case "joinchannel":
					this.event("OnJoinedChannel");
					break ;
				case "tochannelother":
					this.event("OnChannelMsg",dataO);
					break ;
				}
			this.event("OnServerMsg",dataO);
		}
		...
		__proto.sendToChannel=function(obj){
			var dataO;
			dataO={};
			dataO.type="tochannelother";
			dataO.data=obj;
			this.sendJson(dataO);
		}

		__proto.sendTypedToChannel=function(type,obj){
			this.sendToChannel(MsgConst.createMsg(type,obj));
		}
		...
		StockSocket.Logined="Logined";
		StockSocket.OnServerMsg="OnServerMsg";
		StockSocket.Welcome="welcome";
		StockSocket.OnJoinedChannel="OnJoinedChannel";
		StockSocket.OnChannelMsg="OnChannelMsg";
		return StockSocket;
	})(EventDispatcher)
	...
	*@author ww
	*/
	//class remotedebugtool.client.DebugControlClient extends remotedebugtool.client.DebugClient
	var DebugControlClient=(function(_super){
		function DebugControlClient(){
			DebugControlClient.__super.call(this);
		}

		__class(DebugControlClient,'remotedebugtool.client.DebugControlClient',_super);
		return DebugControlClient;
	})(DebugClient)
	...
	*@author ww
	*/
	//class remotedebugtool.client.DebugServer extends remotedebugtool.client.DebugClient
	var DebugServer=(function(_super){
		function DebugServer(){
			DebugServer.__super.call(this);
		}

		__class(DebugServer,'remotedebugtool.client.DebugServer',_super);
		var __proto=DebugServer.prototype;
		__proto.dealChannelMsg=function(dataO){
			_super.prototype.dealChannelMsg.call(this,dataO);
			switch(dataO.type){
				case "GetAllDebugView":
		...
		__proto.updateNodeProps=function(dataO){
			var rootO;
			rootO=dataO.root;
			var nodeO;
			nodeO=dataO.node;
	...
	*/
	...
	var TestPageUI=(function(_super){
		...
		__class(TestPageUI,'remotedebugtool.ui.test.TestPageUI',_super);
	...
	*@author ww
	*/
	//class remotedebugtool.view.EditorViewItem extends remotedebugtool.ui.remotedebugtool.EditorViewItemUI
	var EditorViewItem=(function(_super){
		function EditorViewItem(){
			this.tDataO=null;
			EditorViewItem.__super.call(this);
		}

		__class(EditorViewItem,'remotedebugtool.view.EditorViewItem',_super);
		var __proto=EditorViewItem.prototype;
	...
	*@author ww
	*/
	//class remotedebugtool.view.EditorViewList extends remotedebugtool.ui.remotedebugtool.EditorViewListUI
	var EditorViewList=(function(_super){
		function EditorViewList(){
			this._client=null;
			EditorViewList.__super.call(this);
			this._client=new DebugControlClient();
			this._client.on("OnReady",this,this.onControlReady);
			this._client.on("OnRemoteInfo",this,this.onRemoteMsg);
			this.viewList.array=[];
			this.viewList.renderHandler=new Handler(this,this.listRenderHandler);
			this.freshBtn.visible=false;
			this.freshBtn.on("click",this,this.onFreshBtn);
		}

		__class(EditorViewList,'remotedebugtool.view.EditorViewList',_super);
		var __proto=EditorViewList.prototype;
		__proto.connect=function(server){
			this._client.connect(server);
		}
		...
		__proto.onFreshBtn=function(){
			this._client.sendTypedToChannel("GetAllDebugView",{});
		}

		__proto.onControlReady=function(){
			this.freshBtn.visible=true;
		}

		__proto.onRemoteMsg=function(dataO){
			console.log("onRemoteMsg:",dataO);
			switch(dataO.type){
				case "GetAllDebugView":
					debugger;
					this.viewList.array=dataO.data;
					break ;
				}
		}

		return EditorViewList;
	})(EditorViewListUI)


	//class view.TestView extends remotedebugtool.ui.test.TestPageUI
