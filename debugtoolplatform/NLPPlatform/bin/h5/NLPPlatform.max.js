...
(function(window,document,Laya){
	...
	*/
	//class NLPPlatform
	var NLPPlatform=(function(){
		function NLPPlatform(){
			Laya.init(800,600);
		...
		var __proto=NLPPlatform.prototype;
	...
	*@author ww
	*/
	//class nodetools.devices.Paths
	var Paths=(function(){
		function Paths(){}
		__class(Paths,'nodetools.devices.Paths');
		Paths.tempPath=null
		Paths.dataPath=null
		return Paths;
	})()
	...
	*@author ww
	*/
	//class nodetools.devices.PythonTools
	var PythonTools=(function(){
		function PythonTools(){}
		__class(PythonTools,'nodetools.devices.PythonTools');
		PythonTools.callPython=function(cmdStr,complete){
			cmdStr="C:/Python34/python.exe"+" "+PythonTools.PythonFolder+cmdStr;
			CMDShell.execute(cmdStr,callBackTmp=function(err,stdOut,stdErr){
				console.log("err:",err);
				console.log("stdErr:",stdErr);
				console.log("stdOut:",CMDShell.decode(stdOut,"gbk"));
				if (complete){
					complete.run();
				}
			})
		...
		PythonTools.PythonExePath="C:/Python34/python.exe";
		PythonTools.PythonFolder=null
		PythonTools.GetFolders="autoPy.py";
		PythonTools.CloseSame="autoCloseSame.py";
		PythonTools.CloseAll="autoClose.py";
		return PythonTools;
	})()
	...
	*/
	...
	var TaggingBook=(function(){
		...
		__proto.deleteRelation=function(startWord){
			startWord.head=-1;
		}

		__proto.createRelation=function(startWord,endWord){
		...
		__proto.buildWordRelationByRange=function(startIndex,endIndex){}
	...
	*/
	//class commoncomponent.InputSelect extends laya.ui.ComboBox
	var InputSelect=(function(_super){
		...
		__class(InputSelect,'commoncomponent.InputSelect',_super);
		var __proto=InputSelect.prototype;
		/**@inheritDoc */
		__proto.createChildren=function(){
			_super.prototype.createChildren.call(this);
			this._input=new CommonInput();
			this._input.left=0;
			...
			this.visibleNum=15;
			this._input.on("blur",this,this.onInputEvents,["blur"]);
			this.on("change",this,this.onInputEvents,["blur"]);
		}

		__proto.onInputEvents=function(event){
			this.event(event);
		}
		...
		__getset(0,__proto,'labels',function(){
			return _super.prototype._$get_labels.call(this);
			},function(value){
			if (value)this._labels=value.split(",");
			else this._labels.length=0;
			this._itemChanged=true;
	...
	var BookReader=(function(_super){
		...
		__proto.onBtnClick=function(e){
			...
			switch(tButton.name){
				...
				case "创建关系":
					if (BookReaderState.startWord&&BookReaderState.endWord){
						success=this.book.createRelation(BookReaderState.startWord.dataO,BookReaderState.endWord.dataO);
						this.wordView.showLine();
						}else{
						if (BookReaderState.startWord){
							success=this.book.deleteRelation(BookReaderState.startWord.dataO);
							this.wordView.showLine();
						}
					}
					break ;
	...
	*/
	...
	var WordViewer=(function(_super){
		...
		__proto.onMenuSelect=function(dataO){
			var label;
	...
	*/
	...
	var WordListViewer=(function(_super){
		function WordListViewer(){
			...
			this.lineBox=this.container.content;
			this.idWordDic=new IDDicTool();
		...
		__proto.drawRelation=function(startWord,endWord){
			...
			this.lineBox.graphics.drawCurves(0,0,[startX,startY,startX,midy,midX,midy],"#00ffff");
			this.lineBox.graphics.drawCurves(0,0,[midX,midy,endX,midy,endX,endY],"#ff0000");
			var arrrowL=NaN;
			if (endX < midX){
				arrrowL=30;
				}else{
			...
			this.lineBox.graphics.drawLine(midX,midy,midX+arrrowL,midy,"#ffff00");
			this.lineBox.graphics.fillText(ConllDesParser.getCNType(startWord.dataO.deprel)+"",midX,midy,null,"#ff0000","center");
		}
	...
	*@author ww
	*/
	//class view.nlpplatform.WordProp extends ui.nlpplatform.WordPropUI
	var WordProp=(function(_super){
		function WordProp(){
			this._dataO=null;
			this.wordO=null;
			WordProp.__super.call(this);
		...
		var __proto=WordProp.prototype;
		__proto.setData=function(dataO){
			this._dataO=dataO;
			this.wordO=dataO.dataO;
			this.wordTxt.text=this.wordO.word;
			this.wordType.selectedLabel=this.wordO.type;
			this.meaningInput.text=this.wordO.meaning || "";
			var wordData;
			wordData=WordDicParser.I.cutter.trie.getWord(this.wordO.word);
		...
		__getset(1,WordProp,'I',function(){
			if (!WordProp._i)WordProp._i=new WordProp();
			return WordProp._i;
		},ui.nlpplatform.WordPropUI._$SET_I);

		WordProp.showWordProp=function(dataO){
			WordProp.I.setData(dataO);
			WordProp.I.popup();
		}
	...
	Laya.__init([LoaderManager,EventDispatcher,LocalStorage,Render,Browser,Timer,View,ConllTreeArcAnalyser,GraphicAnimation,PingYinDic]);
	new NLPPlatform();
...
/*
1 file:///E:/onewaymyway/codes/playground/playground/libs/nodetools/src/nodetools/devices/FileManager.as (225):warning:XMLElement This variable is not defined.
...
5 file:///E:/onewaymyway/codes/playground/playground/debugtoolplatform/NLPPlatform/src/view/nlpplatform/BookReader.as (74):warning:success This variable is not defined.
6 file:///E:/onewaymyway/codes/playground/playground/debugtoolplatform/NLPPlatform/src/view/nlpplatform/BookReader.as (81):warning:success This variable is not defined.
*/