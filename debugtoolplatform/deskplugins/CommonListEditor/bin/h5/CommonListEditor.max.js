...
(function(window,document,Laya){
	...
	Laya.interface('commonlayout.mindmaptree.IMindMapTreeItem');
	/**
	*@private
	*/
	...
	*@author ww
	*/
	//class dataeditor.DataTpl
	var DataTpl=(function(){
		function DataTpl(){}
		__class(DataTpl,'dataeditor.DataTpl');
		DataTpl.createDataListData=function(){
			var rst;
			rst={
				"title":"title",
				"type":"type",
		...
		DataTpl.adptDataListData=function(dataO){
	...
	*/
	...
	var MindMapTreeLayouter=(function(){
		...
		__class(MindMapTreeLayouter,'commonlayout.mindmaptree.MindMapTreeLayouter');
		var __proto=MindMapTreeLayouter.prototype;
		...
		__proto.drawConnections=function(treeItem,sprite){
			var childNodes;
			...
			for (i=0;i < len;i++){
				tChild=childNodes[i];
				this.drawConnections(tChild,sprite);
				this.drawOneLine(treeItem,tChild,sprite);
			}
		}

		__proto.getItemHeight=function(tree,childNodes){
			...
			for (i=0;i < len;i++){
				tChild=childNodes[i];
				tHeight=this.getItemHeight(tChild);
				totalHeight+=tHeight;
			}
			totalHeight+=this.YSpace *(len-1);
			return totalHeight;
		}

		__proto.getItemWidth=function(tree,childNodes){
			if (!childNodes)childNodes=tree.childNodes;
		...
		__proto.setPos=function(tree,x,y,isRight){
			(isRight===void 0)&& (isRight=true);
			this.layoutChilds(tree,tree.childNodes,x,y,isRight);
		}

		__proto.layoutAsCenter=function(tree,onlyRight){
			(onlyRight===void 0)&& (onlyRight=false);
			var childNodes;
			childNodes=tree.childNodes;
			if (onlyRight||childNodes.length <=1){
				this.layoutChilds(tree,childNodes,tree.x,tree.y,true,true);
				return;
			};
			var leftChilds;
			var rightChilds;
			...
			this.layoutChilds(tree,leftChilds,tree.x,tree.y,false,true);
			this.layoutChilds(tree,rightChilds,tree.x,tree.y,true,true);
		}

		__proto.layoutChilds=function(parent,childNodes,x,y,isRight,lockPos){
			(isRight===void 0)&& (isRight=true);
			(lockPos===void 0)&& (lockPos=false);
			var itemHeight=NaN;
			itemHeight=this.getItemHeight(parent,childNodes);
			var itemWidth=NaN;
			itemWidth=this.getItemWidth(parent,childNodes);
			...
			var childX=NaN;
			if (isRight){
				childX=x+parent.width+this.XSpace;
				}else{
				childX=x-itemWidth-this.XSpace;
			};
			var tChild;
			var tY=NaN;
			tY=y;
			...
			for (i=0;i < len;i++){
				tChild=childNodes[i];
				childItemHeight=this.getItemHeight(tChild);
				if (isRight){
					this.setPos(tChild,childX,tY,isRight);
					}else{
					this.setPos(tChild,x-tChild.width-this.XSpace,tY,isRight);
				}
				tY+=childItemHeight+this.YSpace;
			}
		}

		return MindMapTreeLayouter;
	})()
	...
	*@author ww
	*/
	//class commontools.uitools.UIGroupControl
	var UIGroupControl=(function(){
		function UIGroupControl(){
			this.items=null;
		}

		__class(UIGroupControl,'commontools.uitools.UIGroupControl');
		var __proto=UIGroupControl.prototype;
		__proto.removeSelf=function(){
			this.execute("removeSelf")
		}

		__proto.addToParent=function(parentO){
		...
		__proto.execute=function(funName,params){
			if (!this.items)return;
			var i=0,len=0;
			len=this.items.length;
			var tItem;
	...
	*/
	...
	/**消息管理器
	*/
	//class commontools.MessageManager extends laya.display.Sprite
	var MessageManager=(function(_super){
		...
		__class(MessageManager,'commontools.MessageManager',_super);
	...
	*/
	...
	var MindMapViewer=(function(_super){
		function MindMapViewer(){
			...
			this._dataO=null;
			this._root=null;
			MindMapViewer.__super.call(this);
			this._tempPoint=new Point();
			this._layouter=new MindMapTreeLayouter();
		...
		__class(MindMapViewer,'commonlayout.mindmaptree.MindMapViewer',_super);
		var __proto=MindMapViewer.prototype;
		...
		__proto.createNodeByData=function(dataO){
			...
			var tChild;
			for (i=0;i < len;i++){
				tChild=this.createNodeByData(childs[i]);
				MindMapViewer.addChildNode(tItem,tChild);
			}
			this.nodeContainer.addChild(tItem);
			return tItem;
		}
		...
		__proto.clearPreItems=function(){
			...
			for (i=len-1;i >=0;i--){
				...
				if (Laya.__typeof(tChild,'commonlayout.mindmaptree.IMindMapTreeItem')){
					tChild.recover();
				}
			}
		...
		MindMapViewer.addChildNode=function(parent,child){
			parent.childNodes.push(child);
			child.parentNode=parent;
		}

		MindMapViewer.removeChildNode=function(parent,child){
			var childNodes;
	...
	*@author ww
	*/
	//class commonlayout.mindmaptree.MindMapTreeBase extends laya.ui.View
	var MindMapTreeBase=(function(_super){
	...
	*/
	...
	//class ui.dataeditor.DataItemUI extends laya.ui.View
	var DataItemUI=(function(_super){
		function DataItemUI(){
			this.label=null;
			DataItemUI.__super.call(this);
		}

		__class(DataItemUI,'ui.dataeditor.DataItemUI',_super);
		var __proto=DataItemUI.prototype;
		__proto.createChildren=function(){
			View.regComponent("commoncomponent.CommonInput",CommonInput);
			laya.ui.Component.prototype.createChildren.call(this);
			this.createView(DataItemUI.uiView);
		}

		DataItemUI.uiView={"type":"View","props":{"width":74,"height":14},"child":[{"type":"TextInput","props":{"y":-1,"x":1,"width":73,"var":"label","text":"label","skin":"comp/input_22.png","runtime":"commoncomponent.CommonInput","height":15,"color":"#ef3431"}}]};
		return DataItemUI;
	})(View)


	//class ui.dataeditor.DataListUI extends laya.ui.View
	var DataListUI=(function(_super){
		function DataListUI(){
			this.itemList=null;
			this.addBtn=null;
			this.titleTxt=null;
			DataListUI.__super.call(this);
		}

		__class(DataListUI,'ui.dataeditor.DataListUI',_super);
		var __proto=DataListUI.prototype;
		__proto.createChildren=function(){
	...
	*@author ww
	*/
	//class dataeditor.DataItem extends ui.dataeditor.DataItemUI
	var DataItem=(function(_super){
		function DataItem(){
			this.actionHandler=null;
			this._dataO=null;
			DataItem.__super.call(this);
			this.setUpTextInput(this.label,"label");
			this.on("rightmousedown",this,this.onTextMouseDown);
		}

		__class(DataItem,'dataeditor.DataItem',_super);
		var __proto=DataItem.prototype;
	...
	*@author ww
	*/
	//class dataeditor.DataList extends ui.dataeditor.DataListUI
	var DataList=(function(_super){
		function DataList(){
			...
			this.dataO=null;
			DataList.__super.call(this);
			this.itemsControl=new UIGroupControl();
			this.itemsControl.items=[this.addBtn,this.itemList];
			this.itemActionHandler=new Handler(this.onItemAction);
			this.addBtn.on("click",this,this.onAddBtn);
			this.titleTxt.on("doubleclick",this,this.onTitleDoubleClick);
		...
		__proto.onTitleDoubleClick=function(){
			this.switchState();
		}
