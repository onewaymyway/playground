...
(function(window,document,Laya){
	...
	*/
	...
	var Game=(function(){
		function Game(){
			this.wordView=null;
			this.conllParser=null;
			this.wordTreeView=null;
			this.wordTreeView2=null;
			this.wordTreeView3=null;
			this.contreeBuilder=null;
			this.book=null;
		...
		__proto.startWordParserTest=function(){
			...
			this.contreeBuilder=NLP.contreeBuilder;
			this.conllParser=NLP.conllParser;
			this.onDicLoaded();
		}
		...
		__proto.testWordParser=function(){
			this.wordView=new WordListViewer();
			this.wordView.pos(20,20);
			this.wordView.width=Laya.stage.width-40;
			Laya.stage.addChild(this.wordView);
			this.wordTreeView2=new WordTreeViewer();
			this.wordTreeView2.pos(20,300);
			this.wordTreeView2.width=Laya.stage.width-40;
			Laya.stage.addChild(this.wordTreeView2);
			this.wordTreeView3=new WordTreeViewer();
			this.wordTreeView3.pos(20,600);
			this.wordTreeView3.width=Laya.stage.width-40;
			Laya.stage.addChild(this.wordTreeView3);
			WordDicParser.I.cutter.cutToMap("每个人的一生，都离不开金钱、离不开商业，但是，很多人从来没有试图好好地、认真地去走近它，了解它。它是一个我们每天都碰到的、陌生的朋友。");
			this.wordTreeView2.setTree(NLP.contreeBuilder.cutStringToTree("每个人的一生，都离不开金钱、离不开商业，但是，很多人从来没有试图好好地、认真地去走近它"));
			this.wordTreeView3.setTree(NLP.contreeBuilder.cutStringToTree("每个人的一生，都离不开金钱、离不开商业，但是，很多人从来没有试图好好地、认真地去走近它",true));
			this.testCut();
		}

		__proto.testWordTree=function(){
			NLP.initWordCutter("data/中文字典1.txt","data/CoreNatureDictionary.txt");
			NLP.initConllTreeParser("data/conlldes.txt",["data/text.train.conll","data/news.train.conll"]);
			this.contreeBuilder=NLP.contreeBuilder;
			this.conllParser=NLP.conllParser;
			console.log("contreeBuilder:",this.contreeBuilder);
			this.wordTreeView=new WordTreeViewer();
			this.wordTreeView.pos(20,10);
			this.wordTreeView.width=Laya.stage.width-40;
			Laya.stage.addChild(this.wordTreeView);
			this.wordTreeView.setTree(this.conllParser.getCurLine());
			this.wordTreeView2=new WordTreeViewer();
			this.wordTreeView2.pos(20,300);
			this.wordTreeView2.width=Laya.stage.width-40;
			Laya.stage.addChild(this.wordTreeView2);
			this.wordTreeView2.setTree(this.contreeBuilder.rebuildConllTree(this.conllParser.getCurLine()));
			this.wordTreeView3=new WordTreeViewer();
			this.wordTreeView3.pos(20,600);
			this.wordTreeView3.width=Laya.stage.width-40;
			Laya.stage.addChild(this.wordTreeView3);
			this.wordTreeView3.setTree(this.contreeBuilder.rebuildConllTree(this.conllParser.getCurLine(),true));
			Laya.stage.on("keydown",this,this.onWordTreeKeyDown);
		}

		__proto.onWordTreeKeyDown=function(e){
			switch(e.keyCode){
				case 38:
					this.wordTreeView.setTree(this.conllParser.pre());
					this.wordTreeView2.setTree(this.contreeBuilder.rebuildConllTree(this.conllParser.getCurLine()));
					this.wordTreeView3.setTree(this.contreeBuilder.rebuildConllTree(this.conllParser.getCurLine(),true));
					break ;
				case 40:
					this.wordTreeView.setTree(this.conllParser.next());
					this.wordTreeView2.setTree(this.contreeBuilder.rebuildConllTree(this.conllParser.getCurLine()));
					this.wordTreeView3.setTree(this.contreeBuilder.rebuildConllTree(this.conllParser.getCurLine(),true));
					break ;
				}
		...
		__proto.testCut=function(){
			...
			this.book=BookParser.createByTxt(Loader.getRes("books/qqst.txt"));
			Laya.stage.on("keydown",this,this.onKeyDown);
			this.showTxt("键盘上下控制段落切换显示");
		}
		...
		__proto.showTxt=function(str){
			...
			this.wordView.setWordList(words);
			this.wordTreeView2.setTree(NLP.contreeBuilder.cutStringToTree(str));
			this.wordTreeView3.setTree(NLP.contreeBuilder.cutStringToTree(str,true));
		}
	...
	*/
	...
	var PingYinDic=(function(){
		...
		PingYinDic.inits=function(){
			...
			len="９".charCodeAt(0);
			for (i=start;i <=len;i++){
				PingYinDic.NumCharDic[String.fromCharCode(i)]=true;
				PingYinDic.transDic[String.fromCharCode(i)]=(i-start)+"";
			};
			var numCN0="一二三四五六七八九十百千万亿兆";
			var numCN1="壹贰叁肆伍陆柒捌玖拾佰仟";
			len=numCN0.length;
			for (i=0;i < len;i++){
				PingYinDic.NumCharDicCN[numCN0.charAt(i)]=true;
			}
			len=numCN1.length;
			for (i=0;i < len;i++){
				PingYinDic.transDic[numCN1.charAt(i)]=numCN0.charAt(i);
			}
		}
	...
	*/
	...
	var CharDicCutter=(function(){
		...
		__proto.findMaxWord=function(str,tPos){
			...
			tPiece.word=CharDicCutter.adptWordByTrans(str.substring(tPiece.start,tPiece.end));
		...
		CharDicCutter.hasTransInWOrd=function(word){
			var i=0,len=0;
			len=word.length;
			var tWord;
			for (i=0;i < len;i++){
		...
		CharDicCutter.adptWordByTrans=function(word){
			var words;
			words=word.split("");
			var i=0,len=0;
			len=words.length;
	...
	*/
	...
	var WordCutter=(function(){
		...
		__proto.init=function(){
			...
			this.charDicCutter.addCutDic("NUMBERCN",PingYinDic.NumCharDicCN);
		...
		__proto.getLongerWordPiece=function(wordA,wordB){
		...
		__proto.findMaxWord=function(str,pos){
			...
			var tPiece;
			var dicPiece;
			dicPiece=this.charDicCutter.findMaxWord(str,pos);
			var tchar;
			tchar=str.charAt(pos);
			var tTrieNode;
			tTrieNode=this.trie.findByChar(tchar);
			...
			if (!tTrieNode){
				tPiece.word=tchar;
				tPiece.type="new";
				return this.getLongerWordPiece(dicPiece,this.adptPiece(tPiece));
			}
			if (tTrieNode.isWord()){
				tPiece.end=pos+1;
		...
		__proto.findMaxWordR=function(str,pos){
			...
			var tPiece;
			var dicPiece;
			dicPiece=this.charDicCutter.findMaxWordR(str,pos);
			var tchar;
			tchar=str.charAt(pos);
			var tTrieNode;
	...
	*/
	...
	var ConllTreeScoreUtils=(function(){
		function ConllTreeScoreUtils(){
			...
			this._relationTypeList=[];
			this._relationTypeList.push([KeysCounter.create(),[2,2],1]);
			this._relationTypeList.push([KeysCounter.create(),[1,2],0.01]);
			this._relationTypeList.push([KeysCounter.create(),[2,1],0.01]);
			this._relationTypeList.push([KeysCounter.create(),[1,1],0.0001]);
		}
		...
		__proto.addRelation=function(startWord,endWord){
			...
			len=this._relationTypeList.length;
			for (i=0;i < len;i++){
				tGroupArr=this._relationTypeList[i];
				tCounter=tGroupArr[0];
				tArr=tGroupArr[1];
				tCounter.addKey(this.getAdptWordKey(endWord,tArr[0],startWord),this.getAdptWordKey(startWord,tArr[1],endWord),startWord.deprel);
			}
		}
		...
		__proto.getRelationType=function(startWord,endWord){
			var i=0,len=0;
			len=this._relationTypeList.length;
			var tCounter;
			var tArr;
			...
			for (i=0;i < len;i++){
				...
				tCounter=tGroupArr[0];
				tArr=tGroupArr[1];
				tType=tCounter.getMaxKey(this.getAdptWordKey(endWord,tArr[0],startWord),this.getAdptWordKey(startWord,tArr[1],endWord));
				if (tType){
					debugger;
					return tType;
				}
		...
		__proto.buildWordRelation=function(wordList,useDP,tree){
			...
			for (j=0;j < jLen;j++){
				...
				if (tSelectWord){
					...
					tSelectWord.deprel=this.treeAnalyseer.scoreUtils.getRelationType(tSelectWord,tSelectRWord);
				}
			}
		}
	...
	*/
	...
	var WordForTreeUI=(function(_super){
		function WordForTreeUI(){
			this.txt=null;
			WordForTreeUI.__super.call(this);
		}

		__class(WordForTreeUI,'ui.wordparser.WordForTreeUI',_super);
		var __proto=WordForTreeUI.prototype;
		__proto.createChildren=function(){
			View.regComponent("commoncomponent.AutoSizeLabel",AutoSizeLabel);
			laya.ui.Component.prototype.createChildren.call(this);
			this.createView(WordForTreeUI.uiView);
		}

		WordForTreeUI.uiView={"type":"View","props":{"width":85,"height":29},"child":[{"type":"Label","props":{"y":0,"x":0,"width":85,"var":"txt","text":"文字","runtime":"commoncomponent.AutoSizeLabel","height":29,"fontSize":20,"color":"#f4e4e4"}}]};
		return WordForTreeUI;
	})(View)


	//class ui.wordparser.WordListViewerUI extends laya.ui.View
	var WordListViewerUI=(function(_super){
		...
		__class(WordListViewerUI,'ui.wordparser.WordListViewerUI',_super);
	...
	//class ui.wordparser.WordTreeViewerUI extends laya.ui.View
	var WordTreeViewerUI=(function(_super){
		function WordTreeViewerUI(){
			this.container=null;
			WordTreeViewerUI.__super.call(this);
		}

		__class(WordTreeViewerUI,'ui.wordparser.WordTreeViewerUI',_super);
		var __proto=WordTreeViewerUI.prototype;
		__proto.createChildren=function(){
			laya.ui.Component.prototype.createChildren.call(this);
			this.createView(WordTreeViewerUI.uiView);
		}

		WordTreeViewerUI.uiView={"type":"View","props":{"width":600,"height":400},"child":[{"type":"Box","props":{"var":"container","top":5,"right":5,"left":5,"bottom":5}}]};
		return WordTreeViewerUI;
	})(View)
	...
	var WordViewerUI=(function(_super){
		...
		__class(WordViewerUI,'ui.wordparser.WordViewerUI',_super);
	...
	*/
	...
	var WordForTree=(function(_super){
		function WordForTree(){
			this.dataO=null;
			WordForTree.__super.call(this);
		}

		__class(WordForTree,'view.WordForTree',_super);
		var __proto=WordForTree.prototype;
		...
		__proto.getPos=function(i){
			var tValue=NaN;
			tValue=this.getCenterX();
	...
	*@author ww
	*/
	//class view.WordListViewer extends ui.wordparser.WordListViewerUI
	var WordListViewer=(function(_super){
		...
		__class(WordListViewer,'view.WordListViewer',_super);
	...
	*@author ww
	*/
	//class view.WordTreeViewer extends ui.wordparser.WordTreeViewerUI
	var WordTreeViewer=(function(_super){
		function WordTreeViewer(){
			this.layouter=null;
		...
		__class(WordTreeViewer,'view.WordTreeViewer',_super);
		var __proto=WordTreeViewer.prototype;
		__proto.clearPre=function(){
			if (!this.wordItemList)return;
			var i=0,len=0;
			len=this.wordItemList.length;
		...
		__proto.setTree=function(tree){
			this.tree=tree;
			tree.buildRelation();
			this.setWordList(tree.wordList);
		}
		...
		__proto.freshUI=function(){
			this.relationLayer.graphics.clear();
			this.container.addChild(this.relationLayer);
			if (!this.wordItemList)return;
			this.layouter.layout(this.wordItemList,this.container.width,this.container.height);
			this.drawRelations();
		}
		...
		__proto.drawRelation=function(relation){
			...
			this.relationLayer.graphics.fillText(ConllDesParser.getCNType(relation.type),(startX+endX)*0.5,tY,null,"#ff0000","center");
			this.relationLayer.graphics.drawCircle(endX,tY,2,"#00ff00");
		}

		return WordTreeViewer;
	})(WordTreeViewerUI)
	...
	*@author ww
	*/
	//class view.WordViewer extends ui.wordparser.WordViewerUI
	var WordViewer=(function(_super){
		...
		__class(WordViewer,'view.WordViewer',_super);
