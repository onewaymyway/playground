...
(function(window,document,Laya){
	...
	*/
	...
	var Game=(function(){
		...
		__proto.testWordTree=function(){
			...
			this.conllParser=new ConllFileParser();
			this.conllParser.parseTxt(Loader.getRes("data/text.train.conll"));
			ConllTreeAnalyse.typeDic=typeDic;
			var analyse;
			analyse=new ConllTreeAnalyse();
			analyse.analyse(this.conllParser.treeList);
	...
	*/
	...
	var ConllTree=(function(){
		...
		__proto.buildRelation=function(){
			...
			var tmpWord;
			for (i=0;i < len;i++){
				tWord=this.relations[i];
				for (j=i+1;j < len;j++){
					cpWord=this.relations[j];
					if (tWord.end==cpWord.start){
						this.relations[j]=tWord;
						tWord=cpWord;
					}
				}
			}
	...
	*/
	...
	Laya.__init([LoaderManager,EventDispatcher,PingYinDic,Render,View,Browser,Timer,GraphicAnimation,LocalStorage]);
	new Game();
